...
<div class="channel-engine" style="display: none;">
	...
	<main>
		...
		<input id="ce-status" type="hidden" value="<?php esc_attr( $data['status'] ); ?>">
		<input id="ce-check-status" type="hidden"
			   value="<?php esc_attr( Shop_Helper::get_controller_url( 'Check_Status', 'get_sync_data' ) ); ?>">
		<input id="ce-enable-plugin" type="hidden"
			   value="<?php esc_attr( Shop_Helper::get_controller_url( 'Enable', 'enable' ) ); ?>">
		<input id="ce-check-order-sync" type="hidden"
			   value="<?php esc_attr( Shop_Helper::get_controller_url( 'Check_Status', 'get_order_sync_config' ) ); ?>">
