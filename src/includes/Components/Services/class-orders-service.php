...
class Orders_Service extends OrdersService
{
    ...
    public function create(Order $order)
    {
        try {
            ...
            if ($config->isEnableVatExcludedPrices()) {
                $wc_order->set_total($order->getTotalExclVat());
                $wc_order->set_shipping_total($order->getShippingCostsExclVat());
            } else {
    ...
    protected function get_tax_subtotal_item(Order $order)
    {
        ...
        $total_excluded_vat = $order->getTotalInclVat() - $total_vat;
        if($total_excluded_vat > 0){
            $tax_subtotal->set_rate_percent((100 * $total_vat) / $total_excluded_vat);
        }
    ...
    private function orderFromChannelEngineAlreadyExists(Order $order): bool
    {
        $wc_orders = wc_get_orders(
            array(
                ...
                // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- Empty.
                'meta_value' => $order->getId(),
                // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value -- Empty.
            )
        );
